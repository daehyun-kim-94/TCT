...
import com.lgcns.test.servlet.DlqServlet;
...
public class Main {
	...
	public static final String CMD_DLQ = "DLQ";
	...
	public ResponseMsg doOp(RequestMsg reqMsg) {
		...
		case CMD_CREATE: 
			if(qm.createQueue(reqMsg.getQueueName(), 
					reqMsg.getQueueSize(),
					reqMsg.getProcessTimeout(),
					reqMsg.getMaxFailCount(),
					reqMsg.getWaitTime()
					) != null) {
		...
		case CMD_SEND: 
			...
			break;
		case CMD_RECEIVE: 
			tctMsg = qm.receive(reqMsg.getQueueName());
			if(tctMsg == null) {
		...
		case CMD_FAIL: 
			...
			break;
		case CMD_DLQ: 
			tctMsg = qm.dlq(reqMsg.getQueueName());
			if(tctMsg == null) {
				repMsg.setResult(ResponseMsg.RST_NO_MESSAGE);
			} else {
				repMsg.setMsg(tctMsg.getMsg());
	...
	private Server createServer(int port) {
		...
		DlqServlet dlqServlet = new DlqServlet(this);
		ServletHolder dlqHolder = new ServletHolder(dlqServlet);
		context.addServlet(dlqHolder, createServletPath(CMD_DLQ));		
